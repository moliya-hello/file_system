...
bool Format()    //格式化一个虚拟磁盘文件
{
    ...
    superBlock->s_SUPER_BLOCK_SIZE = sizeof(SuperBlock);
    ...
    superBlock->s_Super_block_StartAddr = superBlockStartAddr;
    ...
    //初始化磁盘块区，根据成组链接法组织
    for (i = BLOCK_NUM / BLOCKS_PER_GROUP - 1;
         i >= 0; i--) {    //一共INODE_NUM/BLOCKS_PER_GROUP组，一组FREESTACKNUM（128）个磁盘块 ，第一个磁盘块作为索引
    ...
    strcpy(cur.i_g_name, Cur_Group_Name);
    ...
    cur.i_in_dir_Block_1 = -1;    //没使用一级间接块
...
void printSuperBlock()        //打印超级块信息
{
    ...
    printf("本系统 block大小：%d 字节，每个inode占 %d 字节（真实大小：%d 字节）\n", superBlock->s_BLOCK_SIZE,
           superBlock->s_INODE_SIZE, sizeof(Inode));
    printf("\t每磁盘块组（空闲堆栈）包含的block数量：%d\n", superBlock->s_blocks_per_group);
    printf("\t超级块占 %d 字节（真实大小：%d 字节）\n", superBlock->s_BLOCK_SIZE, superBlock->s_SUPER_BLOCK_SIZE);
    printf("磁盘分布：\n");
    printf("\t超级块开始位置：%d B\n", superBlock->s_Super_block_StartAddr);
...
{
    printf("\n");
    printf("block（磁盘块）使用表：[used:%d %d/%d]\n", superBlock->s_BLOCK_NUM - superBlock->s_free_BLOCK_NUM,
           superBlock->s_free_BLOCK_NUM, superBlock->s_BLOCK_NUM);
    int i;
    i = 0;
    printf("0 ");
...
int balloc()    //磁盘块分配函数
{
    ...
    fseek(fw, (retAddr - Block_StartAddr) / BLOCK_SIZE + BlockBitmap_StartAddr,
          SEEK_SET);    //(retAddr-Block_StartAddr)/BLOCK_SIZE为第几个空闲块
...
bool mkdir(int parinoAddr, char name[])    //目录创建函数。参数：上一层目录文件inode地址 ,要创建的目录名
{
    ...
    if (posi != -1) {    //找到这个空闲位置
        ...
        strcpy(p.i_g_name, Cur_Group_Name);
...
bool rmdir(int parinoAddr, char name[])    //目录删除函数
{
    ...
    if (strcmp(Cur_User_Name, cur.i_uname) == 0)
        filemode = 6;
    else if (strcmp(Cur_User_Name, cur.i_g_name) == 0)
        filemode = 3;
    else
        filemode = 0;
...
void ls(int parinoAddr)        //显示当前目录下的所有文件和文件夹。参数：当前目录的inode节点地址
{
    ...
    while (i < cnt && i < 160) {
        ...
        for (j = 0; j < 16 && i < cnt; j++) {
            ...
            printf("%s\t", tmp.i_g_name);    //文件所属用户名
            printf("%d B\t", tmp.i_size);    //文件大小
            printf("%d.%d.%d %02d:%02d:%02d  ", 1900 + ptr->tm_year, ptr->tm_mon + 1, ptr->tm_mday,
                   (8 + ptr->tm_hour) % 24, ptr->tm_min, ptr->tm_sec);    //上一次修改的时间
    ...
    while (i < 160) {
        ...
        for (j = 0; j < 16; j++) {
            if (strcmp(dirlist[j].itemName, name) == 0) {
                ...
                if (((tmp.i_mode >> 9) & 1) == 1) {
                    //找到该目录，判断是否具有进入权限
                    if (((tmp.i_mode >> filemode >> 0) & 1) == 0 &&
                        strcmp(Cur_User_Name, "root") != 0) {    //root用户所有目录都可以查看
                        //没有执行权限
                        printf("权限不足：无执行权限\n");
                        return;
                    }
