...
struct SuperBlock {
    ...
    unsigned short s_SUPER_BLOCK_SIZE;        //超级块大小
    unsigned short s_blocks_per_group;        //每 blockgroup 的block数量

    //磁盘分布
    int s_Super_block_StartAddr;
...
struct Inode {
    ...
    __attribute__((unused)) int i_in_dir_Block_1;                        //一级间接块。512B/4 * 512B = 128 * 512B = 64KB
    //unsigned int i_indieBlock_2;			//二级间接块。(512B/4)*(512B/4) * 512B = 128*128*512B = 8192KB = 8MB
    //unsigned int i_indieBlock_3;			//三级间接块。(512B/4)*(512B/4)*(512B/4) * 512B = 128*128*128*512B = 1048576KB = 1024MB = 1G
    //文件系统太小，暂且省略二级、三级间接块
};
...
extern const int superBlockStartAddr;        //超级块 偏移地址,占一个磁盘块
...
bool bfree();                                                    //磁盘块释放函数
int ialloc();                                                    //分配i节点区函数
__attribute__((unused)) bool ifree();                            //释放i结点区函数
